...
describe Question do
  ...
  context "when creating" do
    ...
    it "#part_of_group? and #solo? are aware of question groups" do
      question.question_group = create(:question_group)
  ...
  context "with answers" do
    let(:answer_1){ create(:answer, :question => question, :display_order => 3, :text => "blue")}
    let(:answer_2){ create(:answer, :question => question, :display_order => 1, :text => "red")}
    let(:answer_3){ create(:answer, :question => question, :display_order => 2, :text => "green")}
    before do
  ...
  context "for views" do
    it "#text_for with #display_type == image" do
      ...
      expect(question.text_for).to match(%r{<img .*src="/(images|assets)/rails(?:-[^"]+)?\.png".*/>})
    end
    it "#help_text_for"
